...
function loadTable() {
  $("#experiment-header").empty();
  var id = $.urlParam('id');
  var queryMeta = "/cse-gui/data/table/experiment-meta/" + id;
  $.ajax(queryMeta).done(function(json) {
    var totals = getExpectedCount(json);
    loadExperiment(totals);
  });
}

function loadExperiment(totals) {
  var id = $.urlParam('id');
  var query = "/cse-gui/data/detail/experiment/" + id;
  $.ajax(query).done(function(json) {
    map(new DateFormat([ "date", "last_execution" ]), json);
    map(new DecimalFormat([ "avg_exec_time" ]), json);
    json.total = totals[0];
    json.traces = totals[1]
    json.expected = json.size * json.total;
    json.pct_completed = df((json.executions / json.expected) * 100);
    var data = templates.experimentHeader.render(json);
    
    $("#experiment-header").append(data);
    prettyPrint();
    loadChart(json);
  });
}
...
function doGraph(json, parent) {
  ...
  svg.append("line")
    .attr("x1", xmargin)
      .attr("x2", width - x({i : .1 }) + xmargin)
      .attr("y1", height + voffset)
      .attr("y2", height + voffset);
  var yaxis = svg.append("g")
    ...
    .attr("class", "rule")
    .attr("transform", function(d) {
        return "translate(" + xmargin + "," + (y(d) + voffset) + ")";
      });
  yaxis.append("line")
    .attr("x2", width);
  yaxis.append("text")
