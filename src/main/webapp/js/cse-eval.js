...
function loadTable() {
	...
	$("#tab2").empty();
	var id = $.urlParam('id');
	var queryMeta = "/cse-gui/data/table/stage-meta/" + id;
	$.ajax(queryMeta).done(function(json) {
		var rows = json.rows;
		for ( var i = 0; i < rows.length; i++) {
			loadEvaluation(rows[i].stage);
		}
	});
}

function loadEvaluation(stage) {
	loadF1Eval(stage);
	loadMAPEval(stage);

}

function loadF1Eval(stage) {
	var id = $.urlParam('id');
	var query = "/cse-gui/data/table/f-measure/" + id + "/" + stage;
	$.ajax(query).done(function(json) {
		var data = templates.f1Eval.render(json);
		$('#tab1').append(data);
		$('#f1-table').dataTable({
			"bPaginate" : false,
			"aoColumnDefs" : [ {
...
function loadMAPEval(stage) {
	var id = $.urlParam('id');
	var query = "/cse-gui/data/table/map-measure/" + id + "/" + stage;
	$.ajax(query).done(function(json) {
		var data = templates.mapEval.render(json);
		$("#tab2").append(data);
		$('#map-table').dataTable({
			"bPaginate" : false,
			"aoColumnDefs" : [ {
...
function doGraph(json, parent) {
	...
	var margin = 25, xmargin = 35, width = 760 - xmargin, height = 300 - .5 - margin, voffset = 10, mx = source.length,
	// my = d3.max(data, function(d) {
	// return d3.max(d, function(d) {
	// return d.y0 + d.y;
	// });
	// }),
	my = 100, mz = d3.max(data, function(d) {
		return d3.max(d, function(d) {
			return d.y;
		});
	}), x = function(d) {
		...
		return (height - d.y0 * height / my) + voffset;
	}, y1 = function(d) {
		return (height - (d.y + d.y0) * height / my) + voffset;
	};
	var y = d3.scale.linear().range([ height, 0 ]);
	var svg = d3.select("#progress-chart").append("svg").attr("width",
			width + xmargin).attr("height", height + margin + voffset);
	...
	bars.append("rect").attr("width", x({
		i : .9
	})).attr("x", 0).attr("y", height + voffset).attr("height", 0).transition()
			.delay(function(d, i) {
				return i * 10;
	...
	var labels = svg.selectAll("text.glabel").data(data[0]).enter().append(
			"text").attr("class", "glabel").attr("dy", ".85em").attr(
